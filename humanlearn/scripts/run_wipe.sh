...
do
    ...
    if [ $MODEL == "vanilla" ]; then
        sh tests/scripts/main_wipe.sh $TASK_NAME $LANG_ORDER "cont-mono" "no"
    
    elif [ $MODEL == "incr-joint" ]; then
        sh tests/scripts/main_wipe.sh $TASK_NAME $LANG_ORDER "multi-incr-cll" "no"
    
    elif [ $MODEL == "multi" ]; then
        sh tests/scripts/main_wipe.sh $TASK_NAME $LANG_ORDER "multi" "no"

    elif [ $MODEL == "er-rand" ]; then
        sh tests/scripts/main_wipe.sh $TASK_NAME $LANG_ORDER "cont-mono" "no" $LTN_MODEL "fifo" "yes" 5 "epoch" "er-main" "random"

    elif [ $MODEL == "mer-rand" ]; then
        sh tests/scripts/main_wipe.sh $TASK_NAME $LANG_ORDER "cont-mono" "no" $LTN_MODEL "fifo" "yes" 5 "epoch" "er-main" "equal-lang"
