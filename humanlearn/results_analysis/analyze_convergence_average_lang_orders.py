...
EVAL_TYPE = "test"
TASK_NAME = "MTOP"
ABLATION_MODE = "convergence"  # "convergence" "er-strategies" "er-techniques"
...
def get_mean_tuple(dict_, list=True):
    if list:
        return (
            list(np.mean([dict_[k][0] for k in dict_], axis=0)),
            list(np.mean([dict_[k][1] for k in dict_], axis=0)),
        )
    else:
        return (
            np.mean([dict_[k][0] for k in dict_], axis=0),
            np.mean([dict_[k][1] for k in dict_], axis=0),
        )
...
def compute_metrics(model_name):
    ...
    return (
        get_mean_tuple(avg_forgetting_epochs),
        get_mean_tuple(avg_final_perf_epochs),
        get_mean_tuple(avg_acc_perf_epochs),
        get_mean_tuple(avg_fwt_mono_perf_epochs),
        get_mean_tuple(avg_fwt_perf_epochs),
    )
...
def compute_metrics_best_dev(model_name):
    ...
    for LANG_ORDER in LANGUAGE_ORDERS[TASK_NAME]:
        ...
        avg_forget = forget_avg(metrics_np, LANGS, LANGS)
        avg_forgetting[LANG_ORDER] = (avg_forget[4], avg_forget[5])
        #
        avg_fp = final_perf(metrics_np, LANGS)
        avg_final_perf[LANG_ORDER] = (avg_fp[0], avg_fp[1])
        #
        avg_acc = acc_avg(metrics_np, LANGS, LANGS)
        avg_acc_perf[LANG_ORDER] = (avg_acc[2], avg_acc[3])

        #
        if "Transfer" in METRICS_NAMES:
            avg_fwt_mono = fwt_avg_mono(metrics_np, LANGS, mono_perf[TASK_NAME])
            avg_fwt_mono_perf[LANG_ORDER] = (avg_fwt_mono[2], avg_fwt_mono[3])
        else:
        ...
        if "Zero-Shot Transfer" in METRICS_NAMES:
            avg_fwt = fwt_avg(metrics_np, LANGS, rand_perf[TASK_NAME])
            avg_fwt_perf[LANG_ORDER] = (avg_fwt[2], avg_fwt[3])
        else:
            avg_fwt_perf[LANG_ORDER] = (0.0, 0.0)

    return (
        get_mean_tuple(avg_forgetting, list=False),
        get_mean_tuple(avg_final_perf, list=False),
        get_mean_tuple(avg_acc_perf, list=False),
        get_mean_tuple(avg_fwt_mono_perf, list=False),
        get_mean_tuple(avg_fwt_perf, list=False),
    )
