...
def bootstrap(root_dir, lang_order, ep_ids, model_name):
...
def compute_bootstrap(root_dir, lang_order, ep_ids, model_name):
    acc_array = ([], [])
    forget_array = ([], [])
    fwt_array = ([], [])
    fwt_mono_array = ([], [])
    ...
    for _ in range(n_iters):
        (
            ...
            forget_perf_all,
            final_perf_all,
        ) = bootstrap(root_dir, lang_order, ep_ids, model_name)
...
def compute_metrics_best_dev_bootstrap(model_name):
    ...
    # Find the best idx
    ep_ids = {}
    with open(root_results_dir + LANG_ORDER + "/metrics.json") as file:
        results_json = json.load(file)

    LANGS = LANG_ORDER.split("_")
    for i, lang in enumerate(LANGS):  # training
        best_valid_perf = multiply_two_lists(
            results_json[i]["valid"][acc_alias][str(i)],
            results_json[i]["valid"][slots_alias][str(i)],
        )
        ep_idx_best_val = best_valid_perf.index(max(best_valid_perf))
        ep_ids.update({lang: ep_idx_best_val})
...
if ABLATION_MODE == "convergence":
    ...
    if do_language_order:
        # EVAL_FUNC = compute_metrics_one_lang_order
        EVAL_FUNC = compute_metrics_best_dev_bootstrap
    else:
        EVAL_FUNC = compute_metrics
