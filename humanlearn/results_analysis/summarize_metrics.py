...
def acc_avg(metrics, order, eff_order):
    ...
    if (
        np.mean(
            [
                metrics[len(eff_order) - 1][i_test * 2]
                for i_test, lang_test in enumerate(eff_order)
            ]
        )
        == 0
    ):
    ...
    avg_intent_acc_el = {
        el: sum_intent_acc[el] / len_eff_order for el in sum_intent_acc
    }
...
def bwt_avg(metrics, order, eff_order):
    ...
    for i_train, el_train in enumerate(eff_order):
        ...
        for i_test, el_test in enumerate(order[:i_train]):
            avg_intent_acc_el[el_train] += (
                metrics[i_train][i_test * 2] - metrics[i_test][i_test * 2]
            )
            avg_slot_f1_el[el_train] += (
                metrics[i_train][i_test * 2 + 1] - metrics[i_test][i_test * 2 + 1]
            )
...
def forget_avg(metrics, order, eff_order):
    ...
    for i_train, el_train in enumerate(eff_order):
        ...
        for i_test, lang_test in enumerate(order[:i_train]):
            ...
            max_forget_intent = compute_max_forget(
                [
                    metrics[i_prev][i_test * 2] - curr_intent_perf
                    for i_prev in range(i_train)
                ]
            )

            max_forget_slot = compute_max_forget(
                [
                    metrics[i_prev][i_test * 2 + 1] - curr_slot_perf
                    for i_prev in range(i_train)
                ]
            )
...
def fwt_avg(metrics, order, random_perf):
    fwt_order = order[
        1:
    ]  # we don't look at forward transfer effect on the first element
    avg_intent_acc_el = {el: 0.0 for el in fwt_order}
    avg_slot_f1_el = {el: 0.0 for el in fwt_order}

    for i_test, el_test in enumerate(
        order
    ):  # same as eff_order (but need to keep the original order of indices to match with the metrics results matrix in random_perf)
        if i_test == 0:
            continue
        for i_train, el_train in enumerate(order[:i_test]):
            avg_intent_acc_el[el_test] += (
                metrics[i_train][i_test * 2] - random_perf[el_test][0]
            )
            avg_slot_f1_el[el_test] += (
                metrics[i_train][i_test * 2 + 1] - random_perf[el_test][1]
            )
    ...
    avg_intent_acc_el.update({order[0]: 0.0})

    avg_intent_acc_test = {lang: 0.0 for lang in order}
    avg_slot_f1_test = {lang: 0.0 for lang in order}
    for lang in avg_intent_acc_el:
        avg_intent_acc_test[lang] = avg_intent_acc_el[lang]

    for lang in avg_slot_f1_el:
        avg_slot_f1_test[lang] = avg_slot_f1_el[lang]
...
def fwt_avg_k(metrics, order, random_perf, k):
    fwt_order = order[
        1:
    ]  # we don't look at the forward transfer effect on the first element
    ...
    for i_train, lang_train in enumerate(
        order
    ):  # same as eff_order (but need to keep the original order of indices to match with the metrics results matrix in random_perf)
        ...
        sum_intent_acc += (
            metrics[i_train][(i_train + k) * 2] - random_perf[lang_test][0]
        )
        sum_slot_f1 += (
            metrics[i_train][(i_train + k) * 2 + 1] - random_perf[lang_test][1]
        )
        avg_intent_acc_test[lang_test] += (
            metrics[i_train][(i_train + k) * 2] - random_perf[lang_test][0]
        )
        avg_slot_f1_test[lang_test] += (
            metrics[i_train][(i_train + k) * 2 + 1] - random_perf[lang_test][1]
        )
...
def fwt_avg_mono(metrics, order, mono_perf):
    fwt_order = order[
        1:
    ]  # we don't look at forward transfer effect on the first language
    avg_intent_acc_el = {el: 0.0 for el in fwt_order}
    avg_slot_f1_el = {el: 0.0 for el in fwt_order}

    for i_test, el_test in enumerate(
        order
    ):  # same as eff_order (but need to keep the original order of indices to match with the metrics results matrix)
        if i_test == 0:
            continue
        avg_intent_acc_el[el_test] = metrics[i_test][i_test * 2] - mono_perf[el_test][0]
        avg_slot_f1_el[el_test] = (
            metrics[i_test][i_test * 2 + 1] - mono_perf[el_test][1]
        )
...
def final_perf(metrics, order_lang):
    ...
    intent_acc_lang_dict = {}
    slot_f1_lang_dict = {}
    for i_test, lang_test in enumerate(order_lang):
        intent_acc_lang.append(metrics[end_index][i_test * 2])
        slot_f1_lang.append(metrics[end_index][i_test * 2 + 1])

        intent_acc_lang_dict.update({lang_test: metrics[end_index][i_test * 2]})
        slot_f1_lang_dict.update({lang_test: metrics[end_index][i_test * 2 + 1]})
