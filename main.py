...
from models.GEM import overwrite_grad, project2cone2
...
def nlu_evaluation(model, dataset, dataset_test, nb_examples, use_slots, i, verbose=False):
    ...
    for _ in range(nb_examples):
        ...
        if use_slots:
            ...
            slot_logits = [slot_logits[i, :length].data.cpu().numpy() for i, length in enumerate(lengths)]
...
def train(params_old_tasks,
    ...
    token_type_ids = token_type_ids.cuda()#to(device)
    input_masks = input_masks.cuda()#to(device)
    attention_mask = attention_mask.cuda()#to(device)
    ...
    if args.cont_learn_alg == "ewc":
        ...
        #params_requires_grad = [n for n, p in params.items()]

        #print("i: ", i, " args.freeze_bert:", args.freeze_bert)
        #print("Params which requires_grad:", len(params_requires_grad))
        #if len(params_requires_grad) < 10:
        ...
        #params_all = [n for n, p in model.named_parameters()]
        #print("All Params:", params_all)
        ...
        loss = loss.mean()
        loss.backward()
        store_grads(model.named_parameters(), grads, grad_dims, i)
        if i > 0:
            # forward pass on the previous data stored in the memory
            for old_task_i in range(i): # for all old tasks before current task i
            ...
            indx = torch.LongTensor(list(range(i))).cuda() if torch.cuda.device_count() > 0 \
                else torch.LongTensor(list(range(i)))

            dotp = torch.mm(grads[:, i].unsqueeze(0),
            ...
            if (dotp < 0).sum() != 0:
                # If it is the case:
                # Copy the gradients
                project2cone2(grads[:, i].unsqueeze(1),
                              grads.index_select(1, indx))

                # Update the named_parameters accordingly
                overwrite_grad(model.named_parameters(), grads[:, i], grad_dims)
        saved_grads = {}
...
def run(out_dir, metrics_path, args):
    ...
    #print("args.model_root:", args.model_root)
    #print("os.path.join(args.model_root, model_name):", os.path.join(args.model_root, model_name))
    ...
    if torch.cuda.device_count() > 1:
        num_intents = model.module.num_intents
    else:
        num_intents = model.num_intents

    #print("After addition of OTHER type => num_intents = ", num_intents)
    ...
    if args.setup_opt == "cil":
        ...
        #print("len(dataset.train_stream):", len(dataset.train_stream))

        #for n, p in model.named_parameters():
        #    print("n:", n, " requires_grad:", p.requires_grad)
