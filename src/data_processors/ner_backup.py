...
class NERExample(object):
    ...
    def __repr__(self):
        ...
        return (
            ...
            + self.language
            + " label: "
            + " ".join([NER_TYPES["panx"][label] for label in self.label])
        )
...
def extract_features(tokenizer, words, labels, lang, class_types):
    ...
    for word, label in zip(words, labels):
        ...
        label_ids.extend(
            [class_types.index(label)] + [pad_token_label_id] * (len(word_tokens) - 1)
        )
...
class Processor(object):
    ...
    def next_batch(
        ...
        for example in examples:
            ...
            # lengths_l.append(len(example.input_ids))
            lengths_l.append(example.length)

            max_sent_len = max(max_sent_len, len(example.input_ids))
        ...
        for i in range(1):
            print(
                "BEFORE Checking if length changed here:",
                len(examples[i].input_ids),
                " length:",
                examples[i].length,
            )
        for i in range(len(examples)):
            curr_len_1 = len(input_ids_l[i])
            curr_len = lengths_l[i]

            if curr_len_1 != curr_len:
                print("-------------------------")
                print(
                    " BEFORE input_ids_l[i]: ",
                    input_ids_l[i],
                    " len(input_ids_l[i]):",
                    len(input_ids_l[i]),
                    " curr_len_1: ",
            ...
            input_ids_l[i] += [0] * (max_sent_len - curr_len)
            input_masks_l[i] += [0] * (max_sent_len - curr_len)
            token_type_ids_l[i] += [1] * (max_sent_len - curr_len)
            ...
            #     len(labels_l[i]),
            # )
            assert curr_len == len(labels_l[i])
            labels_l[i] += [0] * (max_sent_len - curr_len)

        for i in range(1):
            print(
                "AFTER Checking if length changed here:",
                len(examples[i].input_ids),
                " length:",
                examples[i].length,
            )
