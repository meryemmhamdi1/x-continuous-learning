...
def compute_multi(root_dir, seed="42"):
    ...
    for i_test, test_lang in enumerate(languages):
        with open(
            prediction_dir + "End_test_perf-train_de-en-es-fr-hi-th-test_" + test_lang,
            "r",
        ) as file:
            results = file.read().splitlines()
...
def compute_mono(root_dir, seed="42", mode="orig"):
    ...
    for i_train, train_lang in enumerate(languages):
        if mode != "orig":
            ...
            prediction_dir = (
                root_dir + "/mono/vanilla/" + train_lang + "/SEED_" + seed + "/"
            )
...
def method_1(root_dir, n, n_iters, model_name):
    ...
    for iter in range(n_iters):
        ...
        for lang_order in lang_orders:
            ...
            results_array = [
                round(forget_perf_all[2], 2),
                round(forget_perf_all[3], 2),
                round(fwt_avg_all[2], 2),
                round(fwt_avg_all[3], 2),
                round(fwt_avg_mono_all[2], 2),
                round(fwt_avg_mono_all[3], 2),
                round(final_perf_all[0], 2),
                round(final_perf_all[1], 2),
            ]
    ...
    return (
        forget_intent_array_all,
        forget_slot_array_all,
        fwt_intent_array_all,
        fwt_slot_array_all,
        fwt_mono_intent_array_all,
        fwt_mono_slot_array_all,
        final_perf_intent_array_all,
        final_perf_slot_array_all,
    )
...
def method_2(root_dir, n, n_iters, model_name, seed):
    ...
    for lang_order in lang_orders:
        ...
        mean_l_f_i, d_l_f_i = compute_statistical_average(
            forget_intent_array_sub, n_iters
        )
        mean_l_f_s, d_l_f_s = compute_statistical_average(
            forget_slot_array_sub, n_iters
        )

        mean_l_fwt_i, d_l_fwt_i = compute_statistical_average(
            fwt_intent_array_sub, n_iters
        )
        mean_l_fwt_s, d_l_fwt_s = compute_statistical_average(
            fwt_slot_array_sub, n_iters
        )

        ###

        mean_l_fwt_1_i, d_l_fwt_1_i = compute_statistical_average(
            fwt_1_intent_array_sub, n_iters
        )
        mean_l_fwt_1_s, d_l_fwt_1_s = compute_statistical_average(
            fwt_1_slot_array_sub, n_iters
        )

        mean_l_fwt_2_i, d_l_fwt_2_i = compute_statistical_average(
            fwt_2_intent_array_sub, n_iters
        )
        mean_l_fwt_2_s, d_l_fwt_2_s = compute_statistical_average(
            fwt_2_slot_array_sub, n_iters
        )

        mean_l_fwt_3_i, d_l_fwt_3_i = compute_statistical_average(
            fwt_3_intent_array_sub, n_iters
        )
        mean_l_fwt_3_s, d_l_fwt_3_s = compute_statistical_average(
            fwt_3_slot_array_sub, n_iters
        )

        mean_l_fwt_4_i, d_l_fwt_4_i = compute_statistical_average(
            fwt_4_intent_array_sub, n_iters
        )
        mean_l_fwt_4_s, d_l_fwt_4_s = compute_statistical_average(
            fwt_4_slot_array_sub, n_iters
        )

        mean_l_fwt_5_i, d_l_fwt_5_i = compute_statistical_average(
            fwt_5_intent_array_sub, n_iters
        )
        mean_l_fwt_5_s, d_l_fwt_5_s = compute_statistical_average(
            fwt_5_slot_array_sub, n_iters
        )

        ###

        mean_l_fwt_mono_i, d_l_fwt_mono_i = compute_statistical_average(
            fwt_mono_intent_array_sub, n_iters
        )
        mean_l_fwt_mono_s, d_l_fwt_mono_s = compute_statistical_average(
            fwt_mono_slot_array_sub, n_iters
        )

        mean_l_fp_i, d_l_fp_i = compute_statistical_average(
            final_perf_intent_array_sub, n_iters
        )
        mean_l_fp_s, d_l_fp_s = compute_statistical_average(
            final_perf_slot_array_sub, n_iters
        )

        per_order_metrics.append(
            [
                model_name,
                lang_order,
                round(mean_l_f_i, 2),
                ...
                round(mean_l_f_s, 2),
                round(d_l_f_s, 2),
                round(mean_l_fwt_i, 2),
                round(d_l_fwt_i, 2),
                round(mean_l_fwt_s, 2),
                round(d_l_fwt_s, 2),
                round(mean_l_fwt_1_i, 2),
                round(d_l_fwt_1_i, 2),
                round(mean_l_fwt_1_s, 2),
                round(d_l_fwt_1_s, 2),
                round(mean_l_fwt_2_i, 2),
                round(d_l_fwt_2_i, 2),
                round(mean_l_fwt_2_s, 2),
                round(d_l_fwt_2_s, 2),
                round(mean_l_fwt_3_i, 2),
                round(d_l_fwt_3_i, 2),
                round(mean_l_fwt_3_s, 2),
                round(d_l_fwt_3_s, 2),
                round(mean_l_fwt_4_i, 2),
                round(d_l_fwt_4_i, 2),
                round(mean_l_fwt_4_s, 2),
                round(d_l_fwt_4_s, 2),
                round(mean_l_fwt_5_i, 2),
                round(d_l_fwt_5_i, 2),
                round(mean_l_fwt_5_s, 2),
                round(d_l_fwt_5_s, 2),
                round(mean_l_fwt_mono_i, 2),
                round(d_l_fwt_mono_i, 2),
                round(mean_l_fwt_mono_s, 2),
                round(d_l_fwt_mono_s, 2),
                round(mean_l_fp_i, 2),
                round(d_l_fp_i, 2),
                round(mean_l_fp_s, 2),
                round(d_l_fp_s, 2),
    ...
    avg_metrics = [
        model_name,
        "average_lang",
        round(np.mean(forget_intent_array_all), 2),
        round(np.std(forget_intent_array_all), 2),
        round(np.mean(forget_slot_array_all), 2),
        round(np.std(forget_slot_array_all), 2),
        round(np.mean(fwt_intent_array_all), 2),
        round(np.std(fwt_intent_array_all), 2),
        round(np.mean(fwt_slot_array_all), 2),
        round(np.std(fwt_slot_array_all), 2),
        round(np.mean(fwt_1_intent_array_all), 2),
        round(np.std(fwt_1_intent_array_all), 2),
        round(np.mean(fwt_1_slot_array_all), 2),
        round(np.std(fwt_1_slot_array_all), 2),
        round(np.mean(fwt_2_intent_array_all), 2),
        round(np.std(fwt_2_intent_array_all), 2),
        round(np.mean(fwt_2_slot_array_all), 2),
        round(np.std(fwt_2_slot_array_all), 2),
        round(np.mean(fwt_3_intent_array_all), 2),
        round(np.std(fwt_3_intent_array_all), 2),
        round(np.mean(fwt_3_slot_array_all), 2),
        round(np.std(fwt_3_slot_array_all), 2),
        round(np.mean(fwt_4_intent_array_all), 2),
        round(np.std(fwt_4_intent_array_all), 2),
        round(np.mean(fwt_4_slot_array_all), 2),
        round(np.std(fwt_4_slot_array_all), 2),
        round(np.mean(fwt_5_intent_array_all), 2),
        round(np.std(fwt_5_intent_array_all), 2),
        round(np.mean(fwt_5_slot_array_all), 2),
        round(np.std(fwt_5_slot_array_all), 2),
        round(np.mean(fwt_mono_intent_array_all), 2),
        round(np.std(fwt_mono_intent_array_all), 2),
        round(np.mean(fwt_mono_slot_array_all), 2),
        round(np.std(fwt_mono_slot_array_all), 2),
        round(np.mean(final_perf_intent_array_all), 2),
        round(np.std(final_perf_intent_array_all), 2),
        round(np.mean(final_perf_slot_array_all), 2),
        round(np.std(final_perf_slot_array_all), 2),
    ]

    CIs = bootstrap.ci(
        fwt_intent_array_all, scipy.mean, n_samples=len(fwt_intent_array_all)
    )
    print(CIs[0], CIs[1])
    ...
    for lang in languages:
        # We want to compute the average over each language order
        mean_l_f_i, d_l_f_i = compute_statistical_average(
            forget_intent_array_lang[lang], len(forget_intent_array_lang[lang])
        )
        mean_l_f_s, d_l_f_s = compute_statistical_average(
            forget_slot_array_lang[lang], len(forget_slot_array_lang[lang])
        )

        forget_lang_string_format.extend(
            [
                lang,
                " & ",
                round(mean_l_f_i, 2),
    ...
    for lang in languages:
        mean_l_fwt_i, d_l_fwt_i = compute_statistical_average(
            fwt_intent_array_lang[lang], len(fwt_intent_array_lang[lang])
        )
        mean_l_fwt_s, d_l_fwt_s = compute_statistical_average(
            fwt_slot_array_lang[lang], len(fwt_slot_array_lang[lang])
        )

        fwt_lang_string_format.extend(
            [
                lang,
                " & ",
                round(mean_l_fwt_i, 2),
    ...
    for lang in languages:
        mean_l_fwt_mono_i, d_l_fwt_mono_i = compute_statistical_average(
            fwt_mono_intent_array_lang[lang], len(fwt_mono_intent_array_lang[lang])
        )
        mean_l_fwt_mono_s, d_l_fwt_mono_s = compute_statistical_average(
            fwt_mono_slot_array_lang[lang], len(fwt_mono_slot_array_lang[lang])
        )

        fwt_mono_string_format.extend(
            [
                lang,
                "&",
                round(mean_l_fwt_mono_i, 2),
...
def multi_purpose_func(model, root_dir, out_dir, seed, languages):
    ...
    for lang in perf:
        perf_summary += (
            lang
            + " "
            + str(round(perf[lang][0], 2))
            + " \pm $"
            + str(round(conf[lang][0], 2))
            + "$ &"
            + str(round(perf[lang][1], 2))
            + " \pm $"
            + str(round(conf[lang][1], 2))
            + "$ &"
        )
    ...
    print(
        " average :"
        + str(round(np.mean([perf[lang][0] for lang in languages]), 2))
        + " \pm $"
        + str(round(np.mean([conf[lang][0] for lang in languages]), 2))
        + "$ &"
        + str(round(np.mean([perf[lang][1] for lang in languages]), 2))
        + " \pm $"
        + str(round(np.mean([conf[lang][1] for lang in languages]), 2))
        + "$"
    )

    dict_pickle = {model + "_perf": perf, model + "_conf": conf}
...
if __name__ == "__main__":
    ...
    mono_ada_tuned_perf, mono_ada_tuned_conf = load_multi_purpose(
        out_dir, "mono_ada_tuned"
    )
    ...
    mono_ada_frozen_perf, mono_ada_frozen_conf = load_multi_purpose(
        out_dir, "mono_ada_frozen"
    )
    ...
    with open(
        os.path.join(res_dir, "all_per_order_metrics_bootstrap.pickle"), "wb"
    ) as file:
