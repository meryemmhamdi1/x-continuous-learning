...
from src.basemodels.transTOD import TransModel
...
def sample_for_q_custom(
    q, task_id, memory, model, sampling_k, sampling_type, dataset, device
):
    task_batches = []
    if sampling_type == "random":
        ...
        for task_num in range(task_id):
            ...
            neighbour_keys = torch.topk(
                distances, sampling_k // task_id, largest=False
            )[1]

            # Get only the indices of the nearest neighbours
            task_batch = (
                input_ids[neighbour_keys],
...
def sample_for_q_custom_many_batches_per_task(
    q, task_id, memory, model, sampling_k, sampling_type, dataset, device, num_batches
):
    task_batches = []
    if sampling_type == "random":
...
class MBPA(object):
    ...
    def forward(self, memory, q, task_id, model, dataset):
        ...
        adaptive_batches = sample_for_q_custom(
            q,
            task_id,
            memory,
            model,
        ...
        base_weights = [
            p for n, p in model.named_parameters() if "trans_model" not in n
        ]

        # Set an adaptive optimizer each time from scratch
        adaptive_optimizer = Adam(
            filter(lambda p: p.requires_grad, self.adaptive_model.parameters()),
    ...
    def forward_reptile_one_batch(self, memory, q, task_id, model, dataset):
        ...
        within_batch_ini_weights = {
            n: p for n, p in model.named_parameters() if "trans_model" not in n
        }
        ...
        for adaptive_batch in adaptive_batches:
            ...
            batch_weights = {
                n: p
                for n, p in self.adaptive_model.named_parameters()
                if "trans_model" not in n
            }
            for n in batch_weights:
                within_batch_ini_weights[n] = within_batch_ini_weights[
                    n
                ] + self.args.beta_reptile * (
                    within_batch_ini_weights[n] - batch_weights[n]
                )
    ...
    def forward_reptile_many_batches(self, memory, q, task_id, model, dataset):
        ...
        for task in range(task_id):
            ...
            adaptive_batches = sample_for_q_custom_many_batches_per_task(
                q,
                task_id,
                memory,
                model,
            ...
            for n in task_weights:
                within_task_ini_weights[n] = within_task_ini_weights[
                    n
                ] + self.args.gamma_reptile * (
                    within_task_ini_weights[n] - task_weights[n]
                )
    ...
    OLD FORWARD
    # Using local parameters different each time for each example reinitialized deepcopy
    # adaptive_model = TransNLU(args=self.args,
    #                              trans_model=self.model_trans,
    #                              num_tasks=self.num_tasks,
    #                              num_intents=self.num_intents,
    #                              eff_num_intents_task=self.eff_num_intents_task,
