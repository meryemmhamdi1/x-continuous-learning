...
class ExtLQNScheduler(object):
    ...
    def __init__(self, init_data, processor_sharing=False):
        ...
        self.deck_of_item = {item: (system, 1) for item, system in self.system_of_item.items()}
        ...
        self.counter_per_item = {item: 0 for item, system in self.system_of_item.items()}

        print("INITIALIZATION OF DECKS self.items_of_deck: ", self.items_of_deck)
        print("self.rep_sched():", self.rep_sched())
    ...
    def next_item(self, current_time=None):
        ...
        d = {k: v for k, v in kvs if v}
        for item, arrival_time in d.items():
        ...
        if self.processor_sharing:
            ...
            sampled = random.choice(list(self.items_of_deck[sampled_deck]))
            return sampled
        else:
    ...
    def update(self, item, outcome, timestamp):
        ...
        if item not in self.deck_of_item:
            self.deck_of_item.update({item: (1, 1)})
            self.counter_per_item.update({item: 0})

        self.counter_per_item[item] += 1
        ...
        print("----- FROM SCHED system:", system, " current_deck:", current_deck, " new_deck:", new_deck)
        if current_deck > 1:
    ...
    def rep_sched(self):
        ...
        """
        new_repr = {}
        for (system, deck_num), items in self.items_of_deck.items():
            items_repr = {}
            for item in items:
            ...
            new_repr.update({(system, deck_num): items_repr})
        return new_repr
