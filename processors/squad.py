...
def _parse_squad(data_path,
    ...
    features = squad_convert_examples_to_features(
                ...
                tokenizer=tokenizer,
                max_seq_length=args.max_seq_length,
                doc_stride=args.doc_stride, # missing
                max_query_length=args.max_query_length,
                is_training=not evaluate,
                return_dataset="pt",
    ...
    process_egs = features
    for i, example in enumerate(examples):
        process_egs_dict.update({i: features[i]})
...
class Processor(object):
    ...
    def read_split(self, lang, split_name):
        ...
        process_egs, process_egs_dict = _parse_squad(file_path + "." + self.data_format,
                                                     self.tokenizer,
                                                     self.args,
                                                     split_name,
                                                     lang)
    ...
    def next_batch(self, batch_size, data_split=None, identifier=None):
        if identifier:
            features = [self.get_item_by_id(identifier)]
        else:
