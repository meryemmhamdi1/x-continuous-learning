...
FREEZE_FIRST=${13:-"yes"}
SEED=${14:-42}
CONT_COMP=${15:-"trans_model gclassifier slot_classifier"}
ADAPTER_TYPE=${16:-"MADX"}
STORING_TYPE=${17:-"reservoir"}
...
TOD_DATASET=${34:-"mtop"}
DATA_FORMAT=${35:-"txt"}
FIFO_RAND_MODE=${36:-"fifo"}
LTN_SCHEDULER_TYPE=${37:-"ltn"}
ER_LQ_SCHEDULER_TYPE=${38:-"main"} # er
RAND_OPTION=${39:-"no"}
...
USE_RAND_PARAMS=""
if [ $RAND_OPTION == "yes" ]; then
   USE_RAND_PARAMS+="--random_pred "
fi
...
echo "    TOD_DATASET:" $TOD_DATASET
echo "    DATA_FORMAT:" $DATA_FORMAT

python -W ignore src/main.py --trans_model $TRANS_MODEL \
                            --setup_opt $STREAM_SETUP \
                            --seed $SEED \
                            --use_crf \
                            --use_er_only \
                            --cont_comp $CONT_COMP \
                            --data_name $TOD_DATASET \
                            ...
                            $USE_MONO_PARAMS $USE_RAND_PARAMS $LEITNER_QUEUES_PARAMS