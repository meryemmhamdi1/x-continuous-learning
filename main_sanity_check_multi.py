...
def nlu_evaluation(model,
    ...
    print("Evaluating on i_task:", i_task)
    if prior_mbert is not None or prior_intents is not None or prior_slots is not None:
        model_dict = model.state_dict()

    if prior_mbert is not None:
...
def set_out_dir(args):
    ...
    if args.multi_head_in and not args.multi_head_out:
        model_dir = os.path.join(model_dir, "multi_head_in_"+args.emb_enc_lang_spec)
...
def run(out_dir, metrics_path, args):
    ...
    if args.setup_opt == "cil":
    ...
    elif args.setup_opt == "cll" or args.setup_opt == "multi-incremental-lang":
        """
        ...
        if args.multi_head_in:
            if args.emb_enc_lang_spec == "all":
                ...
                prior_mbert = [{k: v for k, v in model_trans_alias.from_pretrained(os.path.join(args.model_root, model_name))
                                   .named_parameters() if args.emb_enc_lang_spec in k}
                               for _ in dataset.train_stream]
        ...
        for i, subtask in enumerate(dataset.train_stream):
            ...
            if args.multi_head_in:
                if args.emb_enc_lang_spec == "all":
                    ...
                    trans_model_dict = {"trans_model."+k: v for k, v in original_mbert.named_parameters()
                                        if args.emb_enc_lang_spec in k}
            ...
            if args.multi_head_in:
                prior_mbert[i] = {k: v for k, v in mbert_task.named_parameters() if args.emb_enc_lang_spec in k}

            if args.multi_head_out:
                prior_intents[i] = {k: v for k, v in intents_task.named_parameters()}
                prior_slots[i] = {k: v for k, v in slots_task.named_parameters()}

            # print("After training on language: ", i)
            ...
            for lang in dataset.test_stream:
                metrics[lang] = evaluate_report(dataset.test_stream[lang],
                                                ...
                                                verbose=args.verbose,
                                                prior_mbert=prior_mbert[t_index],
                                                prior_intents=prior_intents[t_index],
                                                prior_slots=prior_slots[t_index])
                t_index += 1
    ...
    elif args.setup_opt == "multi-incremental-lang1":
        """
